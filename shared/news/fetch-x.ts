 ...
 */
export async function fetchX(secrets: NewsSecrets = {}): Promise<FetchXResult> {
	...
	for (const account of X_ACCOUNTS) {
		try {
			...
			for (const tw of tweetData.data ?? []) {
				...
				const title = await translateToJa(titleOriginal, account.lang, secrets);
				const summary = await translateToJa(text, account.lang, secrets);
				items.push({
					id: hashId(url),
					url,
					kind: 'x',
