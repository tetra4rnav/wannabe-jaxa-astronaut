...
export async function fetchFeeds(secrets: NewsSecrets = {}): Promise<NewsItem[]> {
	...
	for (const feed of FEEDS) {
		...
		try {
			...
			for (const item of raw) {
				const title = await translateToJa(item.titleOriginal, item.lang, secrets);
				const summary = await translateToJa(item.summaryOriginal, item.lang, secrets);
				results.push({
					...item,
					titleJa: title.text,
