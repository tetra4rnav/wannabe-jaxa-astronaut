...
| **News item** | Collected official feed / X post. Displayed in project timelines; not a wiki citation. LLM may mark `ingestAsSource` for corpus eligibility. |
...
2. The LLM must not invent project slugs outside the catalog; outputs are filtered to the catalog (empty → `unassigned`).
